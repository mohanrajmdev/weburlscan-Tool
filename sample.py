...
abuseApiKeys = "ENTER_ABUSE_APIKEY"
virustotalApiKeys = "ENTER_VIRUSTOTAL_APIKEY"
...
argParser.add_argument("-V", "--version", action="store_true", help="Prints current version of the script and exit.",
                       required=False)
...
argParser.add_argument("-vr", "--virustotalreports", type=int,
                       help="Virustotal malicious reports threshold. Default 1.", required=False, default=1,
                       metavar="[Threshold number]")
argParser.add_argument("-ar", "--abusereports", type=int, help="Abuse IP DB malicious reports threshold. Default 1.",
                       required=False, default=1, metavar="[Threshold number]")
argParser.add_argument("-p", "--path", type=str,
                       help="Provide path of the txt file contains targets. Default 'target-list.txt'.", required=False,
                       default="target-list.txt")
argParser.add_argument("-t", "--threads", type=int,
                       help="Use multiple threads. It is recommended to use it if you have licenced API keys. Also cli output is buggy when used. I recommend using it with -r.",
                       required=False, default=1, metavar="[Threads]")
argParser.add_argument("-w", "--whois", action="store_true", help="Run whois analysis on the provided targets.",
                       required=False)
...
if args.whois:
    ...
    def check_domain_info( domains):
        ...
        for domain in domains:
            api_url = f"{api_endpoint}?domainName={domain}&username=th3inspector7&password=th3inspector7&outputFormat=JSON"
...
def checkAbuse(value, counter):
    try:
        if ipPattern.search(value):
            ...
            ispName, usageType = (
            "Can only check valid IP addresses on AbuseIPDB", "Can only check valid IP addresses on AbuseIPDB")
...
def checking(value, counter):
    ...
    if (int(isMalicious) >= args.virustotalreports or int(
            reportCount) >= args.abusereports or reportScore == 100) and usageType != "Reserved":
...
if __name__ == "__main__":
    ...
    '''
            html_file.write(
                f"<h1>DomainCheckr Report</h1><h3>Issued on {formatted_time}</h3><h4>Successfully checked on {successfullyChecked} values out of {totalCounter}!</h4><h5> Arguments issued: {sys.argv}</h5>")
            html_file.write(html_css)
            for ip_html in htmlReport:
                html_file.write(ip_html)

            html_file.write(
                f"<table><br><tr><td><h4>Malicious Values</h4> (at least {str(args.virustotalreports)} reported on Virus Total or {str(args.abusereports)} on AbuseIPDB)</td></tr>")
